...
namespace textureCompositor
{
    ...
    public class Compositer : IDisposable
    {
        ...
        RenderTarget2D targetA, targetB;
        ...
        public Compositer(int twidth, int theight,int cwidth,int cheight)
        {
            ...
            targetA = RenderTarget2D.New(gd, twidth, theight, 1, PixelFormat.R8G8B8A8.UNorm);
            targetB = RenderTarget2D.New(gd, twidth, theight, 1, PixelFormat.R8G8B8A8.UNorm);
        ...
        public void Composite( string outputpath, params string[] paths)
        {
            try
            {
                ...
                TextureData[] tdata = new TextureData[gridvolume];
                ...
                for (int x = 0; x < gridwidth; x++)
                {
                    for (int y = 0; y < gridheight; y++)
                    {
                            ...
                            if (File.Exists(paths[index]))
                            {
                                try
                                {
                                    ...
                                    {//Render to targetB
                                        bside = !bside;
                                        gd.ResetTargets();
                                        gd.SetRenderTargets(targetB);
                                        gd.Clear(Color.Transparent);

                                        sb.Begin();

                                        sb.Draw(targetA, targetrect, Color.White);
                                    ...
                                    {//render to targetA
                                        bside = !bside;
                                        gd.ResetTargets();
                                        gd.SetRenderTargets(targetA);
                                        gd.Clear(Color.Transparent);

                                        sb.Begin();

                                        sb.Draw(targetB, targetrect, Color.White);
                                        sb.Draw(celltex, new RectangleF(x * quad.Width, y * quad.Height, quad.Width, quad.Height), Color.White);
                                    ...
                                    cfs.Close();
                                    tdata[index] = new TextureData(paths[index], new TCoord() {u = _u * x,v =  _v * y }, index);
                        ...
                        else tdata[index] = new TextureData("",new TCoord() {u = _u * x,v =  _v * y }, index);
                        

                    }
                }

                TextureDataCollection tdc = new TextureDataCollection(Path.GetFileName(outputpath),_u,_v,tdata);
                TextureDataCollection.WriteCollection(outputpath, tdc);
                ...
                {//save targetB
                    ...
                    {
                        targetB.Save(fs, ImageFileType.Png);
                    }
                }
                else
                { //saveTargetA
                    using (FileStream fs = File.Open(outputtexpath, FileMode.Create, FileAccess.ReadWrite))
                    {
                        targetA.Save(fs, ImageFileType.Png);
                    }
                }
