...
namespace MapCreate
{
    public class MapProject
    {
        ...
        MapSurface[] cells;
        /// <summary>
        /// gets the surface at the coordinates of the grid
        /// </summary>
        /// <param name="x">the x coordinate </param>
        /// <param name="y">the y coordinate </param>
        /// <returns>a surface fro mthe grid</returns>
        MapSurface GetSurface(int x , int y)
        { 
            if((x < gridsize.Width && x > -1) && (y < gridsize.Height && y > -1))
            return cells[x +(y * gridsize.Width)];
            else throw new IndexOutOfRangeException("x + (y * width) = " + (x +(y * gridsize.Width)) + "\r\nGrid size is " + gridArea);
        }

        /// <summary>
        /// gets a copy of the cells surface data
        /// </summary>
        /// <returns> "Surface Array" (Surface[]) </returns>
        public Surface[] GetSurfaces()
        { return cells.Select(a=> a.getSurface()).ToArray(); }
        ...
        public MapProject(string tdp,int GridWidth,int GridHeight,int CellSize = 24)
        {
            ...
            for (int _u = 0; _u < gridsize.Width; _u++)
            {
                ...
                {                    
                    ...
                    cells[index] = new MapSurface(newcell);
                }
            }
        }
        ...
        public void ForCells(Action<MapSurface> act)
        {
